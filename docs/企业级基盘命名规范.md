...
| 层级 | 组件 | 资源类型 | 角色/说明 | 命名示例 | 环境/序号 |
|------|------|----------|-----------|----------|-----------|
| sys | KubeEdge | Deployment | 核心服务 | dep-edge-cloudcore | dev/prod |
| sys | KubeEdge | Service | 核心服务入口 | svc-edge-cloudcore | dev/prod |
| sys | KubeEdge | PVC | 配置存储 | pvc-edge-config | 0/1/... |
| mid | PostgreSQL | StatefulSet | HA 数据库 | sts-pg-ha | dev/prod |
| mid | PostgreSQL | Service | 主节点 | svc-pg-primary | dev/prod |
| mid | PostgreSQL | Service | 从节点 | svc-pg-replica | dev/prod |
| mid | PostgreSQL | PVC | 数据存储 | pvc-pg-data | 0/1/... |
| mid | Redis | StatefulSet | 集群 | sts-redis-cluster | dev/prod |
| mid | Redis | Service | 主节点 | svc-redis-master | dev/prod |
| mid | Redis | PVC | 数据存储 | pvc-redis-data | 0/1/... |
| app | n8n | Deployment | Engine | dep-n8n-engine | dev/prod |
| app | n8n | Service | API | svc-n8n-api | dev/prod |
| app | n8n | PVC | 共享存储 | pvc-n8n-shared | 0/1/... |
| app | Dify | Deployment | Server | dep-dify-server | dev/prod |
| app | Dify | Deployment | Workers | dep-dify-workers | dev/prod |
| app | Dify | Service | API | svc-dify-api | dev/prod |
| app | Dify | PVC | 存储 | pvc-dify-storage | 0/1/... |
| mod | Ollama | Deployment | Engine | dep-ollama-engine | dev/prod |
| mod | Ollama | Service | Endpoint | svc-ollama-endpoint | dev/prod |
| mod | Ollama | PVC | 模型库 | pvc-ollama-lib | 0/1/... |
| mod | DeepSeek | Deployment | LLM Engine | dep-deepseek-llm | dev/prod |
| mod | DeepSeek | Service | API | svc-deepseek-api | dev/prod |
| mod | DeepSeek | PVC | 模型存储 | pvc-deepseek-models | 0/1/... |
